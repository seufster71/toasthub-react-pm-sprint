...
class PMSprintContainer extends Component {
	constructor(props) {
		super(props);
		this.state = {pageName:"PM_SPRINT",isDeleteModalOpen: false, errors:null, warns:null, successes:null};
	...
	onListLimitChange(fieldName) {
		return (event) => {
			...
			this.props.actions.listLimit({state:this.props.pmsprint,listLimit});
		};
	}

	onPaginationClick(value) {
		return(event) => {
			fuLogger.log({level:'TRACE',loc:'SprintContainer::onPaginationClick',msg:"fieldName "+ value});
			let listStart = this.props.pmsprint.listStart;
			let segmentValue = 1;
			let oldValue = 1;
			if (this.state["PM_SPRINT_PAGINATION"] != null && this.state["PM_SPRINT_PAGINATION"] != ""){
				oldValue = this.state["PM_SPRINT_PAGINATION"];
			}
			if (value === "prev") {
			...
			listStart = ((segmentValue - 1) * this.props.pmsprint.listLimit);
			this.setState({"PM_SPRINT_PAGINATION":segmentValue});
			
			this.props.actions.list({state:this.props.pmsprint,listStart});
		};
	}
	...
	searchClick(fieldName,event) {
		let searchCriteria = [];
		if (fieldName === 'PM_SPRINT-SEARCHBY') {
			if (event != null) {
				for (let o = 0; o < event.length; o++) {
					let option = {};
					option.searchValue = this.state['PM_SPRINT-SEARCH'];
					option.searchColumn = event[o].value;
					searchCriteria.push(option);
				}
			}
		} else {
			for (let i = 0; i < this.props.pmsprint.searchCriteria.length; i++) {
				let option = {};
				option.searchValue = this.state['PM_SPRINT-SEARCH'];
				option.searchColumn = this.props.pmsprint.searchCriteria[i].searchColumn;
				searchCriteria.push(option);
			}
		}

		this.props.actions.search({state:this.props.pmsprint,searchCriteria});
	}

	onOrderBy(selectedOption) {
		return (event) => {
			fuLogger.log({level:'TRACE',loc:'SprintContainer::onOrderBy',msg:"id " + selectedOption});
			let orderCriteria = [];
			if (event != null) {
				for (let o = 0; o < event.length; o++) {
					let option = {};
				...
				let option = {orderColumn:"PM_SPRINT_TABLE_NAME",orderDir:"ASC"};
				orderCriteria.push(option);
			}
			this.props.actions.orderBy({state:this.props.pmsprint,orderCriteria});
		};
	}
	
	onSave() {
		return (event) => {
			fuLogger.log({level:'TRACE',loc:'SprintContainer::onSave',msg:"test"});
			let errors = utils.validateFormFields(this.props.pmsprint.prefForms.PM_SPRINT_FORM,this.props.pmsprint.inputFields);
			
			if (errors.isValid){
				this.props.actions.saveItem({state:this.props.pmsprint});
			} else {
	...
	onModify(item) {
		return (event) => {
			...
			fuLogger.log({level:'TRACE',loc:'SprintContainer::onModify',msg:"test"+id});
	...
	onDelete(item) {
		return (event) => {
			fuLogger.log({level:'TRACE',loc:'SprintContainer::onDelete',msg:"test"});
			this.setState({isDeleteModalOpen:false});
			if (item != null && item.id != "") {
				this.props.actions.deleteItem({state:this.props.pmsprint,id:item.id});
			}
		};
	}
	...
	onEditRoles(item) {
		return (event) => {
			fuLogger.log({level:'TRACE',loc:'SprintContainer::onEditRoles',msg:"test"+item.id});
	...
	onCancel() {
		return (event) => {
			fuLogger.log({level:'TRACE',loc:'SprintContainer::onCancel',msg:"test"});
			this.props.actions.list({state:this.props.pmsprint});
		};
	}
	...
	onBlur(field) {
		return (event) => {
			fuLogger.log({level:'TRACE',loc:'SprintContainer::onBlur',msg:field.name});
					...
					if (optionalParams.onBlur.validation != null && optionalParams.onBlur.validation == "matchField") {
						if (field.validation != "") {
							...
							if (validation[optionalParams.onBlur.validation] != null && validation[optionalParams.onBlur.validation].id != null){
								if (this.props.pmsprint.inputFields[validation[optionalParams.onBlur.validation].id] == this.props.pmsprint.inputFields[fieldName]) {
	...
	clearVerifyPassword() {
	//	return (event) => {
			fuLogger.log({level:'TRACE',loc:'SprintContainer::clearVerifyPassword',msg:"Hi there"});
			this.setState({errors:null, successes:null});
			this.props.actions.clearField('PM_SPRINT_FORM_VERIFY_PASSWORD');
	//	}
	}
	...
	render() {
		...
		if (this.props.pmsprint.isModifyOpen) {
			return (
				...
				item={this.props.pmsprint.selected}
				inputFields={this.props.pmsprint.inputFields}
				appPrefs={this.props.appPrefs}
				itemPrefForms={this.props.pmsprint.prefForms}
				onSave={this.onSave}
